...
def _news_feed_subqueries(auditor_user_id=None, supervisor_scope_names=None):
    ...
    a_sql = f"""
        SELECT
            ...
            COALESCE(technicians.employee_code, '') AS technician_code,
            COALESCE(technicians.name, audits.technician_display_name, '') AS technician_name,
    ...
    b_sql = f"""
        SELECT
            ...
            COALESCE(audit_items.photo_path, '') AS photo_path,
            COALESCE(technicians.employee_code, '') AS technician_code,
            COALESCE(technicians.name, audits.technician_display_name, '') AS technician_name,
            COALESCE(auditor_users.username, audits.auditor_name, '') AS auditor_name,
            COALESCE(audit_items.non_compliance_reason, audit_items.item_label, '') AS motivo,
    ...
    c_where.append("LOWER(COALESCE(qc_items.status, '')) IN ('nc_menor', 'nc_mayor')")
    ...
    c_sql = f"""
        SELECT
            ...
            COALESCE(qc_items.photo_path, '') AS photo_path,
    ...
    try:
        ...
        if row is not None:
            if isinstance(row, dict):
                total_raw = row.get("total") or row.get(0)
            else:
                total_raw = row[0] if hasattr(row, "__getitem__") and len(row) > 0 else None
